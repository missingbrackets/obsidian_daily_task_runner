...
from core.task_writer import toggle_task
    ...
    with st.expander(f"{age_label} ({len(tasks)} tasks)", expanded=(age_label == "2+ weeks")):
        for t in tasks:
            ...
            with col2:
                ...
                st.caption(f"{days} days overdue")
            with col3:
                if st.button("✅ Done", key=f"done_{t.file_path}_{t.line_number}"):
                    toggle_task(t, TaskStatus.DONE)
                    st.success(f"Marked done: {t.description[:40]}...")
                    st.info("Re-scan vault to refresh.")
                if st.button("❌ Cancel", key=f"cancel_{t.file_path}_{t.line_number}"):
